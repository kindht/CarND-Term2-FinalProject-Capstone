...
LOOKAHEAD_WPS = 50 # Orig: 200, Number of waypoints we will publish. You can change this number
PUB_RATE   = 20      # Orig: 50Hz, publishing frequency
...
class WaypointUpdater(object):
    ...
    def loop(self):
        rate = rospy.Rate(PUB_RATE)  # 50 out of road , 10 stop
    ...
    def get_closest_waypoint_idx(self):
        ...
        if self.waypoints_tree:
            closest_idx = self.waypoints_tree.query([x, y], 1)[1] # [1] return the index 
        
            # Get closest waypoint's coordinate and its previous coordinate
            ...
            if val > 0:  # take next one if closeat waypoint is behind 
    ...
    def decelerate_waypoints(self, waypoints, closest_idx):
        ...
        for i, wp in enumerate(waypoints):
            ...
            vel = math.sqrt( 2 * MAX_DECEL * dist)  + (i * SMOOTH_DECEL) # add linear deceleration
            if vel < 1.0 :
                vel = 0.0
    ...
    def traffic_cb(self, msg):
        ...
        #if (msg.data != -1) :
           # rospy.logwarn("in waypoint_updater traffic_cb: self.stopline_wp_idx={}".format(msg.data))
